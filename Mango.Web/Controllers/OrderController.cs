    ...
    public class OrderController : Controller
    {
        ...
        public IActionResult GetAll(string status)
        {
            ...
            if (response != null && response.IsSuccess)
            {
                ...
                {
                    case "approved":
                        list = list.Where(e => e.Status == SD.Status_Approved).ToList();
                        break;
                    case "readyforpickup":
                        list = list.Where(e => e.Status == SD.Status_ReadyForPickup).ToList();
                        break;
                    case "cancelled":
                        list = list.Where(e => e.Status == SD.Status_Cancelled).ToList();
                        break;
                    default:
                        break;
                }
